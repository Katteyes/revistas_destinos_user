...
export default function Hero() {
  ...
  return (
    ...
    >
      {/* Carrusel */}
      <div
        className="relative overflow-hidden shadow-lg mx-auto w-full max-w-screen-lg sm:rounded-lg md:rounded-lg lg:rounded-xl"
      ...
      >
        ...
        >
          {importantContents.map(content => (
            <div key={content.id} className="min-w-full flex flex-col relative" style={{ height: '309px'}}>
              <Link to={`/contenido/${content.slug}`} className="block w-full h-80 md:h-[500px] overflow-hidden">
                ...
                {/* Fondo sólido y contenedor de descripción */}
          <div
            className="w-full absolute bottom-0 bg-[rgba(255,255,255,0.3)] backdrop-blur-sm p-4 max-h-[50%]"
            >
            <h2
