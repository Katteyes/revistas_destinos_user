...
<nav className="w-full px-5 md:px-32 pt-10" aria-label="Principal">
  ...
  <div className="absolute left-1/2 top-5 transform -translate-x-1/2 -translate-y-1/2 flex items-center gap-3 mb-10">
    ...
    <a href="/" className="h-24 w-28 sm:h-28 sm:w-35 md:h-30 md:w-40 lg:h-40 lg:w-50 flex items-center justify-center">
    ...
    <div className="ml-auto hidden xl:flex">
      <a
        href="/login"
        className="flex text-sm items-center gap-1.5 px-4 py-1.5 hover:bg-[#f8f0e3]/30 bg-[#FFFFFF] border-[0.1rem] border-[#111C85] rounded-3xl transition-all duration-300 whitespace-nowrap "
      >
...
<nav
  className="flex flex-row items-center justify-between gap-5 md:gap-8 top-15 md:top-18 px-1 sm:px-5 bg-transparent relative z-50 p-0 w-full "
  ...
  <ul className="flex flex-row items-center py-3 gap-1 max-w-4xl mx-auto">
    {navigationItems.map((item, index) => (
      ...
      <li className="relative group">
        <a
          href={item.href}
          className={`text-[12px] sm:text-[16px] md:text-[18px] lg:text-[18px] px-1 sm:px-2 md:px-4 lg:px-6 rounded-sm transition-colors duration-300 ${
